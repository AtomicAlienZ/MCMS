<form enctype="multipart/form-data" action="/admin/index.php?plg=shop_manage&cmd=fieldsets&arg[action]=addFieldset" method="post" id="fieldsetForm">
	<h1>
		<span>
			{if $item}
	...
	<div>
		Name *
		<input type="text" name="data[name]" value="{if $item}{$item->getName()}{/if}">
	</div>
	...
	<hr>
	<button data-bind="click: _dump">!!!</button>
</form>
...
{literal}
	function Field (data) {
		...
		this.id = data.id || Field.prototype.idCounter++;

		this.names = {};
		for (var i = 0; i < _GLOBAL_LANGS.length; i++) {
			this.names[_GLOBAL_LANGS[i]] = ko.observable(data['name_'+_GLOBAL_LANGS[i]] || '');
		}

		this.types = _GLOBAL_TYPES;
		this.type = ko.observable(data.type && _GLOBAL_TYPES.indexOf(data.type) >= 0 ? data.type : _GLOBAL_TYPES[0]);
		this.required = ko.observable(data.required || false);
		...
		this.addOption = function () {
			if (this.shouldHaveOptions()) {
				...
				for (var i = 0; i < _GLOBAL_LANGS.length; i++) {
					t.names[_GLOBAL_LANGS[i]] = ko.observable('');
				}
		...
		this._dump = function () {
			...
			for (var i in this.names) {
				if (this.names.hasOwnProperty(i)) {
					console.log('NAME',i,'=>',this.names[i]());
				}
			}
			for (var i in this) {
				if (this.hasOwnProperty(i) && ko.isObservable(this[i]) && i != 'options') {
						...
						for (var j = 0; j < o.length; j++) {
							...
							for (var k in o[j].names) {
								if (o[j].names.hasOwnProperty(k) && ko.isObservable(o[j].names[k])) {
									console.log('NAME', k, '=>', o[j].names[k]());
								}
							}
						}
						console.log('--------------');
